...
class SiteController extends Controller
{
	...
	{
		if (!User::isAdmin() && !User::isCampusAdmin() && !User::isInstituteAdmin() && !!User::isCampusAdmin()) {
	...
	public function actionUpdateStudentProfileImage($campusId = '')
	{
		...
		if (!empty($post)) {
			...
			if (!empty($profile_photo)) {
				$image = Yii::$app->notification->withoutLoginImagekit($profile_photo, 'profile_image/students');
				$studentDetails->profile_photo = $image['url'];
			} else {
				$studentDetails->profile_photo = $oldImage;
			}
	...
	public function actionLogin()
	{
		...
		if ($model->load(Yii::$app->request->post())) {
			...
			if ($user_verification->id != 1 || $user_verification->id != 1 ) {
