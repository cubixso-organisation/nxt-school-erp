...
/* @var $searchModel app\modules\childassessment\models\search\ChildMeritSearch */
...
$this->title = Yii::t('app', 'Child Merits');
...
<div class="card">
    ...
    <?php  if(\Yii::$app->user->identity->user_role==User::ROLE_ADMIN || \Yii::$app->user->identity->user_role==User::ROLE_CAMPUS_ADMIN){ ?>
        <?= Html::a(Yii::t('app', 'Create Child Merit'), ['create'], ['class' => 'btn btn-success']) ?>
        <?php  } ?>
...
<?php 
    $gridColumn = [
        ...
        //     'filter' => \yii\helpers\ArrayHelper::map(ChildMerit::find()->where(['campus_id' => User::getCampusesByUser(Yii::$app->user->identity->id)])->asArray()->all(), 'id', 'campus_id'),
        //     'filterWidgetOptions' => [
    ...
    <?= GridView::widget([
        ...
        'columns' => $gridColumn,
        'pjax' => true,
        'pjaxSettings' => ['options' => ['id' => 'kv-pjax-container-child-merit']],
        'panel' => [
