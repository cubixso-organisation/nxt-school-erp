...
use app\modules\admin\models\search\RoleHasPermissionsSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;

/**
 * RoleHasPermissionsController implements the CRUD actions for RoleHasPermissions model.
 */
class RoleHasPermissionsController extends Controller
{
    ...
    /**
     * Lists all RoleHasPermissions models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new RoleHasPermissionsSearch();
    ...
    /**
     * Displays a single RoleHasPermissions model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new RoleHasPermissions model.
    ...
    /**
     * Updates an existing RoleHasPermissions model.
    ...
    /**
     * Deletes an existing RoleHasPermissions model.
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = RoleHasPermissions::STATUS_DELETE;
            $model->save(false); 
        }
    ...
    public function actionUpdateStatus(){
		...
		if (! empty ( $post ['id'] ) ) {
			$model = RoleHasPermissions::find()->where([
				'id' => $post['id'],
			])->one();
			if(!empty($model)){
    ...
    /**
     * Finds the RoleHasPermissions model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return RoleHasPermissions the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = RoleHasPermissions::findOne($id)) !== null) {
            return $model;
        } else {
