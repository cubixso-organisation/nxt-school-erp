...
public function behaviors()
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'dashboard-notification', 'clear-notification', 'fetch-chart-data', 'fetch-donut-data', 'get-data-for-chart', 'get-attendance-data-for-chart', 'change-themes-color', 'redirect', 'generate-form-link'],
                        
                        'matchCallback' => function () {
                            // --- CRITICAL LOGGING START ---
                            $user = \Yii::$app->user->identity;
                            
                            // Get User Role and ID safely
                            $role = $user ? $user->user_role : 'NOT_AUTHENTICATED';
                            $userId = $user ? $user->id : 'N/A';
                            
                            $checkResult = \app\models\User::isAdmin(); // Assuming namespace app\models\User

                            \Yii::error("DEBUG: Role Check: User ID: {$userId}, DB Role: {$role}, IsAdmin Result: " . ($checkResult ? 'TRUE' : 'FALSE'));
                            // --- CRITICAL LOGGING END ---

                            // Rule 1: This is the original access check
                            return \app\models\User::isAdmin() || \app\models\User::isInstituteAdmin() || \app\models\User::isCampusAdmin()  || \app\models\User::isCampusSubAdmin() || \app\models\User::isLibraryManager() || \app\models\User::isChefWarden();
                        }
                    ],
                    [
                        'allow' => true,
                        'actions' => ['index', 'dashboard-notification', 'clear-notification', 'fetch-chart-data', 'get-data-for-chart', 'get-attendance-data-for-chart', 'redirect', 'generate-form-link'],
                        'matchCallback' => function () {
    ...
    public function actionIndex()
    {
        ...
        if (User::isInstituteAdmin()) {
            ...
            //     ->andWhere(['in', 'campus_id', $getInstituteHasCampusIds])
            //     ->count();
            $data['total_parents'] = ParentDetails::find()
        ...
        if (User::isCampusAdmin()  || User::isCampusSubAdmin()) {
            ...
            $student_details = StudentDetails::find()->where(['campus_id' => $campus_id])->andWhere(['status' => StudentDetails::STATUS_ACTIVE])->count();
            $total_bus_campus = BusDetails::find()->where(['campus_id' => $campus_id])->count();
            $formLink = Yii::$app->urlManager->createAbsoluteUrl(['/site/admission-form', 'campus_id' => $campus_id]);
            ...
            $data['absent_teachers'] = TeacherDetails::find()
                ...
                ->andWhere(['ta.teacher_details_id' => null]) // No attendance record found for the current date
                ->count();
            ...
            $data['payment_details_today'] = PaymentDetails::find()
                ->where(['campus_id' => $campus_id])
                ->andWhere(['like', 'created_on', $today])
                ->sum('paid_amount');
            ...
            $academicYears = PayFees::find()
                ->select(['academic_year_id'])
                ->where(['campus_id' => $campus_id])
                ->groupBy('academic_year_id')
                ->column(); // returns array of academic_year_id

            foreach ($academicYears as $academicYearId) {
                $academicYear = AcademicYears::findOne($academicYearId);

                $feeStructureSum = PayFees::find()
                    ->alias('pf')
                    ->joinWith('feeStructures fs')
                    ->where([
                        'pf.academic_year_id' => $academicYearId,
                        'pf.campus_id' => $campus_id
                    ])
                    ->sum('fs.fee');

                $feesCutSum = PayFees::find()
                    ->where([
                ...
                $totalFee = $feeStructureSum - $feesCutSum;

                // Get all PayFees IDs for this campus and academic year
                $payFeesIds = PayFees::find()
                    ->select('id')
                    ->where([
                        'academic_year_id' => $academicYearId,
                        'campus_id' => $campus_id
                ...
                $collectedFee = PaymentDetails::find()
                    ->where(['in', 'pay_fees_id', $payFeesIds])
                    ->andWhere(['status' => PaymentDetails::status_success])
                    ->sum('paid_amount');

                $academicFeeData[$academicYear->title] = [
                    'total_fee' => $totalFee ?: 0,
                    'collected_fee' => $collectedFee ?: 0,
                    'pending_fee' => ($totalFee ?: 0) - ($collectedFee ?: 0),
                ];
                // var_dump($collectedFee);
                // exit;
            }
        } else {
            $student_details = '';
        ...
        // // Fetch the latest login log entry for the user
        // $loginLog = LoginLogs::find()->where(['ip' => $currentIp])
        //     ->orderBy(['created_on' => SORT_DESC])
        //     ->one();
        // // var_dump($loginLog);exit;
        // if ($loginLog) {
        //     // Check if IP matches and if the status is active
        //     if ($loginLog->status != LoginLogs::STATUS_ACTIVE) {
        //         // If IP doesn't match or status is not active, logout the user
        //         Yii::$app->user->logout();
        //         Yii::$app->session->setFlash('error', 'Session expired due to IP mismatch or inactive session.');
        //         return $this->redirect(['/site/login']); // Redirect to login page
        //     }

        //     // Check if the login was more than 24 hours ago
        //     $loginTime = strtotime($loginLog->created_on);
        //     $currentTime = time();
        //     $timeDifference = $currentTime - $loginTime;
        //     // var_dump($timeDifference);exit;
        //     if ($timeDifference > 24 * 3600) {

        //         // If the last login is more than 24 hours ago, log the user out and deactivate the login log entry
        //         $loginLog->status = LoginLogs::STATUS_INACTIVE;
        //         $loginLog->save(false); // Mark the log as inactive

        //         Yii::$app->user->logout();
        //         Yii::$app->session->setFlash('error', 'Session expired due to inactivity. Please login again.');
        //         return $this->redirect(['/site/login']); // Redirect to login page
        //     }
        // } else {
        //     Yii::$app->user->logout();
        //     Yii::$app->session->setFlash('error', 'Session expired .');
        //     return $this->redirect(['/site/login']);
        // }

        // Allow access if the user is an admin or campus admin

        // If user passes all checks, render the index page
        return $this->render($url, [
            ...
            'academicFeeData' => $academicFeeData,
...
public function actionError()
    {
        ...
        if ($exception !== null) {
            // Check if the exception is the Forbidden error
            if ($exception instanceof \yii\web\ForbiddenHttpException) {
                // If it's a Forbidden error, render a custom view for access denied
                return $this->render('access-denied', ['exception' => $exception]);
            }

            // For all other errors, render a generic error view
            return $this->render('error', ['exception' => $exception]);
        }
        // Default behavior if no exception is available (shouldn't happen here)
    ...
    {
        $data = [];
        $dashboardNotification = DashboardNotification::find()->where(['id' => $id])->one();
        if (!empty($dashboardNotification->url)) {
            $dashboardNotification->is_read = 1;
            $dashboardNotification->save(false);

            return $this->redirect([$dashboardNotification->url]);
        } else {
            return $this->redirect(['index']);
        }
    }
    ...
    public function actionGenerateFormLink()
    {
        $campusId = Yii::$app->user->identity->campus_id ?? null;

        if ($campusId) {
            $formLink = Yii::$app->urlManager->createAbsoluteUrl(['/site/admission-form', 'campus_id' => $campusId]);
            return $this->render('generate-link', ['formLink' => $formLink]);
        }

        throw new \yii\web\ForbiddenHttpException('Campus ID not found for the logged-in user.');
    }
}
