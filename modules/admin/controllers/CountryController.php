...
class CountryController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'create', 'update', 'delete', 'update-status', 'add-campus', 'add-district', 'add-state'],
    ...
    public function actionView($id)
    {
        ...
        $providerState = new \yii\data\ArrayDataProvider([
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if (!empty($model)) {
            $model->status = Country::STATUS_DELETE;
            $model->save(false);
        }
    ...
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('State');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if ((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formState', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
