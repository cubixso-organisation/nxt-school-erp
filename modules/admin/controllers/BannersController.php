...
use app\modules\admin\models\search\BannersSearch;
...
{
    $model = $this->findModel($id);
    $existingImage = $model->image;  // Store the existing image
    ...
    if ($model->loadAll(Yii::$app->request->post())) {
        try {
            ...
            if (!empty($image)) {
                // If a new image is uploaded, upload it via imageKit and update the model
                $uploadedImage = Yii::$app->notification->imageKitUpload($image, 'school_logo');
                $model->image = $uploadedImage['url'];
            } else {
            ...
            Yii::$app->session->setFlash('error', 'An error occurred while updating the banner: ' . $e->getMessage());
        }
    }
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = Banners::STATUS_DELETE;
            $model->save(false); 
        }
