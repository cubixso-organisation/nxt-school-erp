...
use app\modules\admin\models\PocketMoney;
use app\modules\admin\models\PocketMoneySearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;

/**
 * PocketMoneyController implements the CRUD actions for PocketMoney model.
 */
class PocketMoneyController extends Controller
{
    ...
    /**
     * Lists all PocketMoney models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new PocketMoneySearch();
    ...
    /**
     * Displays a single PocketMoney model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new PocketMoney model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($student_id = '')
    {
        $model = new PocketMoney();

        if ($model->loadAll(Yii::$app->request->post())) {
            $campus_id = User::getCampusesByUser(Yii::$app->user->identity->id);
            $model->campus_id = $campus_id;
            $model->academic_year_id = (new BaseCampus)->getCurrentSession($campus_id);
            $model->student_id = $student_id;
            $model->save(false);
            return $this->redirect(['student-details/hostel-students', 'id' => $model->id]);
        } else {
    ...
    /**
     * Updates an existing PocketMoney model.
    ...
    /**
     * Deletes an existing PocketMoney model.
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if (!empty($model)) {
            $model->status = PocketMoney::STATUS_DELETE;
            $model->save(false);
        }
    ...
    /**
     * Finds the PocketMoney model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PocketMoney the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PocketMoney::findOne($id)) !== null) {
            return $model;
        } else {
