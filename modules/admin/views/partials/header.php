...
<style>
   ...
   .slider {
      ...
      border-radius: 34px;
      overflow: hidden;
      /* Ensures everything stays inside the slider */
   }
   ...
   input:not(:checked)+.slider {
      ...
      /* Soft yellow */
      box-shadow: 0 0 5px rgba(245, 179, 0, 0.6);
      /* Subtle glow */
   }

   /* Subtle cloud effect inside the toggle */
   input:not(:checked)+.slider::after {
      ...
      border-radius: 50%;
      opacity: 0.4;
      box-shadow: 30px 0 50px rgba(255, 255, 255, 0.4), -30px 0 50px rgba(255, 255, 255, 0.4);
...
<?php
$common_logo = Url::base() . '/web/nxt_logo1.png';
// echo $common_logo;
// exit;
...
?>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LSKEW7WTS0"></script>
<script>
...
?>
<div class="header">
   <div class="menu-toggle" style="margin: 0 30px">
   ...
   <a href="<?= Yii::$app->homeUrl ?>" class="logo">
      <img src="<?= $common_logo ?>" alt="Logo" style="height:60px;     margin: 0px 0 0 5%;
    padding: 10px;">
   ...
   <ul class="nav user-menu">
      <div class="top-nav-search" style="margin-right:15px;margin-bottom: 6px;">
         <form>
      ...
      <script>
         // Immediately check for the saved theme cookie and apply the dark mode class if needed
         (function() {
            const savedTheme = document.cookie.split('; ').find(row => row.startsWith('theme='));
            const isDarkMode = savedTheme && savedTheme.split('=')[1] === 'dark';

            if (isDarkMode) {
               document.documentElement.classList.add('darkmode');
               document.getElementById('themeToggle').checked = true; // Set the checkbox to "checked" if dark mode
            } else {
               document.getElementById('themeToggle').checked = false; // Ensure it’s unchecked if not dark mode
            }
         })();
      ...
      <li class="nav-item dropdown has-arrow new-user-menus">
         ...
         <div class="dropdown-menu">
            ...
            // echo '<a class="dropdown-item" href="' . Url::toRoute(['/admin/campus/my-campus', 'id' => Yii::$app->user->identity->id]) . '">Campus Details</a>';
            // }
            ?>
            <?php
            $userRole = Yii::$app->user->identity->user_role;
            if ($userRole == User::ROLE_CAMPUS_ADMIN) { // Comparing for equality

               echo '<a class="dropdown-item" href="' . Url::toRoute(['/admin/campus/my-campus', 'id' => Yii::$app->user->identity->id]) . '">Campus Details</a>';
            }
            ?>
            <!-- <a class="dropdown-item" href="<?= Url::toRoute('/admin/campus/my-campus', 'id'); ?>">Campus Details</a> -->
...
<script>
   $(document).ready(function() {
      ...
      function updateData() {
         ...
         const baseUrl = "<?= Url::toRoute(['/admin/dashboard/redirect']) ?>";
         $.ajax({
            url: '<?= $url ?>',
            type: 'GET',
            success: function(response) {
               ...
               if (notifications.status == "OK") {
                  ...
                  notifications.details.forEach(function(notification) {
                     ...
                     notificationsHTML += `<a href="${baseUrl}?id=${notification.id}">`;
                  ...
                  // Assuming notifications.count is the variable holding the count of notifications
                  if (notifications.count > 0) {
                     $('.notification-count').text(notifications.count).show(); // Show the count when it's greater than 0
                  } else {
                     $('.notification-count').hide(); // Hide when the count is not greater than 0
                  }
