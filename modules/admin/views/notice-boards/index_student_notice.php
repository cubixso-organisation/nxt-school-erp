...
<div class="card">
    ...
    <?php  if(\Yii::$app->user->identity->user_role==User::ROLE_ADMIN || \Yii::$app->user->identity->user_role==User::ROLE_CAMPUS_ADMIN){ ?>
        <?= Html::a(Yii::t('app', 'Create Student Notice'), ['create-student'], ['class' => 'btn btn-success']) ?>
        <?php  } ?>
            ...
            [
                ...
                'buttons' => [
                    ...
                    'update' => function ($url, $model) {
                        if (\Yii::$app->user->identity->user_role == User::ROLE_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_CAMPUS_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_MANAGER) {
                            // Change the action URL to 'student-update' and remove the default confirmation
                            return Html::a('<span class="fas fa-pencil-alt" aria-hidden="true"></span>', ['student-update', 'id' => $model->id]);
                        }
                    },
