...
<div class="card">
    ...
    <?php  if (\Yii::$app->user->identity->user_role==User::ROLE_ADMIN || \Yii::$app->user->identity->user_role==User::ROLE_INSTITUTE_ADMIN || \Yii::$app->user->identity->user_role==User::ROLE_CAMPUS_ADMIN) { ?>
        <?= Html::a(Yii::t('app', 'Create Bus Coordinator'), ['bus-details/coordinator-create'], ['class' => 'btn btn-success']) ?>
        <?php  } ?>
        <?= Html::a(Yii::t('app', 'Advance Search'), '#', ['class' => 'btn btn-info search-button d-none']) ?>
    </p>
    <div class="search-form" style="display:none">
        <?=  $this->render('../employee-details/_search', ['model' => $searchModel]); ?>
    </div>
        </div>
    </div>
    <div class="card">
...
<?php
    $gridColumn = [
        ...
        [
            ...
            'label' => Yii::t('app', 'bus Coordinator Name'),
            'value' => function ($model) {
                return $model->employ_name;
             
            },
            'filterType' => GridView::FILTER_SELECT2,
            'filter' => \yii\helpers\ArrayHelper::map(\app\modules\admin\models\EmployeeDetails::find()
            ->where(['campus_id' => User::getCampusesByUser(Yii::$app->user->identity->id)])
            ->asArray()->all(), 'employ_name', 'employ_name'),
            'filterWidgetOptions' => [
                'pluginOptions' => ['allowClear' => true],
            ],
            'filterInputOptions' => ['placeholder' => 'User', 'id' => 'grid-employee-details-search-employ_name']
        ],
        ...
        [
            ...
            'view'=> function ($url, $model) {
                // $url = '/admin/employee-details/view?id='.$model->id;
                $url = Url::toRoute(['/admin/employee-details/view','id'=>$model->id]);
            ...
            'update'=> function ($url, $model) {
                $url = 'bus-coordinator-update?id='.$model->id;
                if (\Yii::$app->user->identity->user_role == User::ROLE_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_CAMPUS_ADMIN || \Yii::$app->user->identity->user_role == User::role_campus_sub_admin) {
                    return Html::a('<span class="fas fa-pencil-alt" aria-hidden="true"></span>', $url);
                }
             },
            'delete'=> function ($url, $model) {
                $url =Url::toRoute(['bus-coordinator-delete','id'=>$model->id]);
                if (\Yii::$app->user->identity->user_role == User::ROLE_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_CAMPUS_ADMIN|| \Yii::$app->user->identity->user_role == User::role_campus_sub_admin) {
