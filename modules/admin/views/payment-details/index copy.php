...
<?php
    $gridColumn = [
        ...
        [
                ...
                'filter' => \yii\helpers\ArrayHelper::map(\app\modules\admin\models\PayFees::find()->where(['campus_id'=>(new User())->getCampusesByUser(Yii::$app->user->identity->id)])
                    ->asArray()->all(), 'id', function($model){
                           $id = $model['id'];
                        $PayFees  = PayFees::find()->where(['id'=>$id])->one();

                        return  $PayFees->feeStructures->title;
