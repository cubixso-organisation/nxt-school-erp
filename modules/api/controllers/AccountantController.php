...
class AccountantController extends BKController
{
    public function behaviors()
    {
        return ArrayHelper::merge(parent::behaviors(), [
            ...
            'corsFilter' => [
                ...
                'cors' => [
                    // restrict access to
                    'Origin' => ['http://localhost:*', 'http://localhost:50674','http://localhost:49810'],
        ...
        if (!empty($post)) {
            ...
            if ($send_otp['Status'] == 'Success') {
                ...
                if ($auth) {
                    ...
                    $check = User::find()->where(['contact_no'=>$number])->andWhere(['status'=>User::STATUS_ACTIVE])->andWhere(['user_role'=>User::ROLE_ACCOUNTANT])->one();
                    if (empty($check)) {
                        $model = new User();
                        $model->username = $number.'@accountant.com';
                        ...
                        $model->user_role = User::ROLE_ACCOUNTANT;
