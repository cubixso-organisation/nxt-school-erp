...
use app\modules\documentgenerator\models\search\BonafideCertificateSearch;
...
/**
 * BonafideCertificateController implements the CRUD actions for BonafideCertificate model.
 */
class BonafideCertificateController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'update', 'pdf', 'update-status', 'view-certificate', 'preview', 'generate-certificate', 'get-sections', 'get-student-data', 'generate-pdf', 'view-generated-certificate', 'index-certificate-list'],
                        'matchCallback' => function () {
    ...
    /**
     * Lists all BonafideCertificate models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new BonafideCertificateSearch();
    ...
    /**
     * Displays a single BonafideCertificate model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new BonafideCertificate model.
    ...
    /**
     * Updates an existing BonafideCertificate model.
    ...
    /**
     * Deletes an existing BonafideCertificate model.
    ...
    public function actionDelete($id)
    {
        ...
        $model = $this->findModel($id);
        if(!empty($model)){
            $model->status = BonafideCertificate::STATUS_DELETE;
            $model->save(false); 
        }
    ...
    public function actionUpdateStatus(){
		...
		if (! empty ( $post ['id'] ) ) {
			$model = BonafideCertificate::find()->where([
				'id' => $post['id'],
			])->one();
			if(!empty($model)){
    ...
    /**
     * Finds the BonafideCertificate model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return BonafideCertificate the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = BonafideCertificate::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    {
        $model = new \app\modules\documentgenerator\models\BonafideCertificate();
    ...
    {
        $post = Yii::$app->request->post();
        $classId = $post["BonafideCertificate"]["id"];
        $sectionId = $post["BonafideCertificate"]["section_name"];
        $certificateid = $post["BonafideCertificate"]["certificate_name"];
    ...
    public function actionGeneratePdf($studentDetailId, $certificateId)
    {
        try {
            ...
            $certificate = BonafideCertificate::find()->where(['id' => (int)$certificateId])->one();
            ...
            if (!is_dir($uploadDirectory)) {
                FileHelper::createDirectory($uploadDirectory, 0775, true);
            }
            ...
            $pdf = new Pdf([
                ...
                'filename' => $filePath,
                'content' => $this->renderPartial('_printpage', [
                ...
                'cssInline' => 'body { 
                    background-image: url("' . $certificate->background_image . '"); 
