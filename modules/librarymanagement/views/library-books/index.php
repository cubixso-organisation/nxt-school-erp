...
/* @var $searchModel app\modules\librarymanagement\models\search\LibraryBooksSearch */
...
$this->title = Yii::t('app', 'Library Books');
        ...
        <div class="card-body">
            ...
            <p>
                <?php if (\Yii::$app->user->identity->user_role == User::ROLE_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_CAMPUS_ADMIN || \Yii::$app->user->identity->user_role == User::ROLE_LIBRARIAN) { ?>
                    <?= Html::a(Yii::t('app', 'Create Library Books'), ['create'], ['class' => 'btn btn-success']) ?>
                <?php  } ?>
        ...
        <div class="card-body">
            ...
            $gridColumn = [
                ...
                [
                    ...
                    'value' => function ($model) {
                        if ($model->rackNumber) {
                            return $model->rackNumber->rack_number;
                        } else {
                            return NULL;
                        }
                    },
                    'filterType' => GridView::FILTER_SELECT2,
                    'filter' => \yii\helpers\ArrayHelper::map(\app\modules\librarymanagement\models\LibraryRacks::find()->asArray()->all(), 'id', 'id'),
                    'filterWidgetOptions' => [
                        'pluginOptions' => ['allowClear' => true],
                    ],
                    'filterInputOptions' => ['placeholder' => 'Library racks', 'id' => 'grid-library-books-search-rack_number']
                ],
            ...
            <?= GridView::widget([
                ...
                'columns' => $gridColumn,
                'pjax' => true,
                'pjaxSettings' => ['options' => ['id' => 'kv-pjax-container-library-books']],
                'panel' => [
