...
<div class="card">
    ...
    <?php  if(\Yii::$app->user->identity->user_role==User::ROLE_ADMIN || \Yii::$app->user->identity->user_role==User::ROLE_CAMPUS_ADMIN){ ?>
        <?= Html::a('Create Exam Student Marksheet', ['create'], ['class' => 'btn btn-success']) ?>
        <?php  } ?>
...
<?php 
    $gridColumn = [
        ...
        [
                ...
                'filterInputOptions' => ['placeholder' => 'User', 'id' => 'grid-exam-student-marksheet-search-user_id']
            ],
        ...
        [
                ...
                'label' => 'Exam',
                'value' => function($model){                   
                    return $model->exam->id;                   
                },
                'filterType' => GridView::FILTER_SELECT2,
                'filter' => \yii\helpers\ArrayHelper::map(\app\modules\admin\models\Exams::find()->asArray()->all(), 'id', 'id'),
                'filterWidgetOptions' => [
                    'pluginOptions' => ['allowClear' => true],
                ],
